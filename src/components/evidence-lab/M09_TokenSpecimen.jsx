...
// Contrast ratios below are verified with WCAG relative luminance.
// Day ratios are checked against panel #14171D; night ratios against #05080C.
// This module keeps the numbers visible so accessibility can be reviewed,
// instead of being treated as a vague design claim.
...
const STRINGS = {
    en: {
        ...
        colSample: 'Sample',
        colDay: 'Day contrast',
        colNight: 'Night contrast',
        night: 'Night-shift',
        typeRule: 'Sizing heuristic: larger viewing distance needs a larger key value. This is used as a practical design rule, not a universal law.',
        distances: [
            { d: '40 cm', ctx: 'Desk view', px: 30 },
            { d: '1 m', ctx: 'Standing view', px: 46 },
            { d: '3 m', ctx: 'Wall / glance view', px: 72 },
        ],
        sample: '4.2 bar',
        secondary: 'loop A nominal',
        densityTitle: 'Density mode',
        desk: 'Desk · 40 px',
        gloved: 'Gloved · 56 px',
        targetNote: 'Minimum touch target in this mode',
        clusterAck: 'Acknowledge',
        clusterReduce: 'Reduce',
        clusterHold: 'Hold',
        motionCols: ['Intent', 'Duration', 'Easing', 'Preview'],
        motion: [
            {
                intent: 'State change',
                dur: '240 ms',
                ease: 'standard',
                cb: 'cubic-bezier(.22,1,.36,1)',
            },
            {
                intent: 'Alert enters',
                dur: '160 ms',
                ease: 'ease-out',
                cb: 'cubic-bezier(0,0,.2,1)',
            },
            {
                intent: 'Recovery feedback',
                dur: '400 ms',
                ease: 'decelerate',
                cb: 'cubic-bezier(0,0,.2,1)',
            },
        ...
        before: 'before',
        after: 'after',
        motionPaused: 'Motion is reduced — showing before and after frames instead of animation.',
        notes: [
            {
                tag: 'Decision / 設計判斷',
    ...
    zh: {
        ...
        colNight: '夜間對比',
        night: '夜班模式',
        typeRule: '字級經驗法則：觀看距離越遠，關鍵數值就需要越大。這裡把它當成實務設計規則，而不是絕對定律。',
        distances: [
            { d: '40 cm', ctx: '桌面監控', px: 30 },
            { d: '1 m', ctx: '站立查看', px: 46 },
            { d: '3 m', ctx: '牆面／一眼辨識', px: 72 },
        ],
        sample: '4.2 bar',
        secondary: '迴路 A 正常',
        densityTitle: '密度模式',
        desk: '桌面 · 40 px',
        gloved: '手套 · 56 px',
        targetNote: '此模式的最小可點擊範圍',
        clusterAck: '確認',
        clusterReduce: '降流量',
        clusterHold: '長按',
        motionCols: ['用途', '時長', '緩動', '預覽'],
        motion: [
            {
                intent: '狀態變化',
                dur: '240 ms',
        ...
        before: '前',
        after: '後',
        motionPaused: '已降低動態效果——改以動畫前後兩幀呈現。',
        notes: [
            {
                tag: 'Decision / 設計判斷',
            ...
            {RAMP.map(k => {
                ...
                return (
                    <div className="gx-m09-ramp" key={k}>
                        <span className="gx-m09-ramp-name">
                            <StatusIcon name={s.icon} size={14} style={{ color: s.hex }} /> {s.label[lang]}
                        </span>

                        <code className="gx-m09-ramp-hex">
                            {s.hex}
                        </code>

                        <span
                            className="gx-m09-ramp-chip"
                            style={{ background: '#14171D', color: s.hex }}
                        >
                            Aa 4.2
                        </span>

                        <span className="gx-m09-ramp-ratio">
                            {s.ratio}:1
                        </span>
                        ...
                        <span className="gx-m09-ramp-night">
                            <span
                                className="gx-m09-ramp-chip"
                                style={{ background: '#05080C', color: s.night }}
                            >
                                Aa
                            </span>
...
function GlanceTypeTab({ t }) {
    return (
        <div className="gx-m09-type">
            {t.distances.map(d => (
                <div className="gx-m09-type-row" key={d.d}>
                    <div className="gx-m09-type-meta">
                        <span className="gx-m09-type-dist">
                            {d.d}
                        </span>
                        <span className="gx-m09-type-ctx">
                            {d.ctx}
                        </span>
                    </div>
            ...
            <p className="gx-caption gx-m09-type-rule">
                {t.typeRule}
            </p>
        </div>
    );
...
function DensityTab({ t }) {
    ...
    return (
        <div className="gx-m09-density">
            <div
                className="gx-m09-density-toggle"
                role="radiogroup"
                aria-label={t.densityTitle}
            >
                <button
                    role="radio"
                    aria-checked={!gloved}
                    className={`gx-m09-dbtn${!gloved ? ' on' : ''}`}
                    onClick={() => setGloved(false)}
                >
                    {t.desk}
                </button>
            ...
            <div className="gx-m09-cluster" style={{ '--hit': `${px}px` }}>
                <button className="gx-m09-cbtn primary">
                    {t.clusterAck}
                </button>

                <button className="gx-m09-cbtn">
                    {t.clusterReduce}
                </button>

                <div className="gx-m09-stepper">
                    <button className="gx-m09-cbtn" aria-label="decrease">
                        −
                    </button>

                    <span>70%</span>

                    <button className="gx-m09-cbtn" aria-label="increase">
                        +
                    </button>
                </div>
            </div>

            <p className="gx-caption">
                {t.targetNote}: <b style={{ color: 'var(--gx-teal)' }}>{px} px</b>
            </p>
        </div>
    );
}
...
function MotionTab({ t, reduced }) {
    ...
    return (
        <div className="gx-m09-motion">
            {reduced && (
                <p className="gx-caption gx-m09-motion-note">
                    {t.motionPaused}
                </p>
            )}

            <div className="gx-m09-motion-head">
                {t.motionCols.map((c, i) => (
                    <span key={i}>
                        {c}
                    </span>
                ))}
            ...
            {t.motion.map((m, i) => (
                <div className="gx-m09-motion-row" key={m.intent}>
                    <span className="gx-m09-motion-intent">
                        {m.intent}
                    </span>
                    ...
                    <code className="gx-m09-motion-ease">
                        {m.ease}
                    </code>
                    ...
                    <div className="gx-m09-motion-demo">
                        {reduced ? (
                            <div className="gx-m09-frames">
                                <span className="gx-m09-frame">
                                    <i style={{ opacity: 0.3 }} />{t.before}
                                </span>
                            ...
                            <>
                                <div className="gx-m09-track">
                                    <span
                                        className="gx-m09-dot"
                                        style={{
                                            transform: playing === i ? 'translateX(56px)' : 'translateX(0)',
                                            opacity: playing === i ? 1 : 0.4,
                                            transitionDuration: m.dur,
                                            transitionTimingFunction: m.cb,
                                        }}
                                    />
                                </div>

                                <button className="gx-btn gx-m09-play" onClick={() => play(i)}>
                                    ▸ {t.play}
                                </button>
                            </>
                        )}
    ...
    return (
        ...
        >
            <div className="gx-m09">
                ...
                >
                    {t.tabs.map((label, i) => (
                        <button
                            ...
                            id={`gx-m09-tab-${i}`}
                            aria-selected={tab === i}
                            aria-controls={`gx-m09-panel-${i}`}
                            tabIndex={tab === i ? 0 : -1}
                            ref={el => (tabsRef.current[i] = el)}
                            className={`gx-m09-tab${tab === i ? ' active' : ''}`}
                            onClick={() => setTab(i)}
                        >
                            <span className="gx-m09-tab-num">
                                0{i + 1}
                            </span>{' '}
                            {label}
                        </button>
                ...
                <div
                    className="gx-m09-panel"
                    role="tabpanel"
                    id={`gx-m09-panel-${tab}`}
                    aria-labelledby={`gx-m09-tab-${tab}`}
                    tabIndex={0}
                >

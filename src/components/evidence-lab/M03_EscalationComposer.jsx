...
const STRINGS = {
    en: {
        ...
        title: 'Alert escalation strategy composer: When to notify, when to interrupt',
        intent: 'This module uses event severity and evidence confidence together to determine the alert level. Low-risk or low-confidence events remain as background cues; as risk and evidence increase, the interface progressively escalates to banners, audible prompts, or interruptive confirmation. The point is not to make every alert more conspicuous, but to decide which situations warrant interrupting the user.',
        severity: 'Severity',
        ...
        rungs: ['Status badge', 'Banner', 'Banner with sound prompt', 'Interruptive confirmation'],
        ...
        friction: {
            ...
            typed: 'type the specified text',
        },
        live: {
            off: 'no announcement',
            polite: 'non-interrupting',
            assertive: 'immediate voice announcement',
        },
        caption: 'Alert levels should be determined by risk and evidence together, not by simply adding volume or visual stimulation.',
        readout: (severity, confidence, rung, live, friction) =>
            `When event severity is ${severity} and evidence confidence is ${confidence}, the alert escalates to ${rung}, paired with ${live}; the user must complete ${friction} before returning to the workflow.`,
        alertBody: {
        ...
        notes: [
            {
                tag: 'Decision / 設計判斷',
                text: 'A low-confidence critical signal should not immediately take over the whole screen. If the system frequently interrupts while evidence is insufficient, users gradually lose trust in alerts and may ignore the events that truly need attention.',
            },
            {
                tag: 'Trade-off / 設計取捨',
                text: 'Typed confirmation deliberately adds friction. When an event has major consequences and subsequent action is difficult to reverse, this delay prompts users to reconsider the state and consequences rather than dismiss the alert reflexively.',
            },
        ],
    },
    zh: {
        eyebrow: '互動邏輯',
        title: '警報升級策略編排器：何時提示，何時打斷',
        intent: '這個模組以「事件嚴重度」與「證據可信度」共同決定警報層級。低風險或低可信度事件維持在背景提示；隨著風險與證據增加，介面才逐步升級為橫幅警示、聲音提醒或阻斷式確認。重點不是讓所有警報更醒目，而是判斷哪些情境值得打斷使用者。',
        severity: '嚴重度',
        confidence: '證據可信度',
        sev: ['注意', '警告', '重大', '危急'],
        conf: ['低', '中', '高'],
        ...
        caption: '警報層級應由風險與證據共同決定，而不是單純增加音量或視覺刺激。',
        readout: (severity, confidence, rung, live, friction) =>
            `當事件嚴重度為「${severity}」，且證據可信度為「${confidence}」時，警報升級為「${rung}」，搭配「${live}」；使用者須完成「${friction}」後才能返回工作流程。`,
        alertBody: {
        ...
        notes: [
            {
                tag: 'Decision / 設計判斷',
                text: '低可信度的危急訊號不應立即接管整個畫面。若系統在證據不足時仍頻繁中斷操作，使用者會逐漸降低對警報的信任，甚至忽略真正需要處理的事件。',
            },
            {
                tag: 'Trade-off / 設計取捨',
                text: '輸入確認刻意增加了一段操作摩擦。當事件影響重大、後續處置又難以回復時，這段延遲能促使使用者重新確認狀態與後果，而不是反射性地關閉警報。',
            },
        ],
    },
};

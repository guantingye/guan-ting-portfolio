...
const SKILLS = [
    {
        ...
        en: { name: 'Sourcing', input: 'Company name / signal', method: 'Pull public filings, funding databases, and reporting; de-dupe against existing DB rows.', output: 'Evidence pack, every claim source-tagged', guard: 'No source, no claim — nothing gets written without a citation.' },
        zh: { name: '檢索', input: '公司名 / 訊號', method: '拉公開申報、募資資料庫與報導；與既有資料庫列去重比對。', output: '帶來源標記的證據包', guard: '無源不寫——沒有引用的主張不會被寫進去。' },
    },
    {
        id: 'S2', key: 'MOAT-DECOMP', section: 'moat',
        en: { name: 'Moat decomposition', input: 'Evidence pack', method: 'Classify moat type (technical / data / distribution / regulatory / capital) and rate durability.', output: 'THE MOAT section + type tag', guard: 'Must separate a real moat from a temporary lead.' },
    ...
    },
    {
        ...
        zh: { name: '商模對映', input: '證據包', method: '從公開訊號推論定價機制、GTM 型態與追加銷售面。', output: 'BUSINESS MODEL 段', guard: '不確定的推論需標記，不可寫成事實。' },
    },
    {
        id: 'S4', key: 'FUNDING-RECON', section: 'funding',
        en: { name: 'Funding reconstruction', input: 'Evidence pack', method: 'Reconstruct rounds, investors, and valuation trajectory from filings and reporting.', output: 'FUNDING STATUS section', guard: 'Amounts as ranges with dates — never a false-precise number.' },
        zh: { name: '資金重建', input: '證據包', method: '從申報與報導重建輪次、投資人與估值軌跡。', output: 'FUNDING STATUS 段', guard: '金額給區間並附日期——絕不給假精確的數字。' },
    },
    {
        id: 'S5', key: 'RISK-FRAMING', section: 'risks',
        en: { name: 'Risk framing', input: 'S2–S4 outputs', method: 'Classify into competitive / technical / capital / talent / platform risk.', output: 'KEY RISKS section (enumerated)', guard: 'Must surface at least one non-obvious risk, not just the visible one.' },
        zh: { name: '風險框架', input: 'S2–S4 輸出', method: '分類為競爭／技術／資本／人才／平台五類風險。', output: 'KEY RISKS 段（編號）', guard: '至少涵蓋一個非顯而易見的風險，而不只是表面那個。' },
    },
    {
        id: 'S6', key: 'VERDICT-SCORE', section: 'verdict',
        en: { name: 'Verdict scoring', input: 'All prior outputs', method: 'Score track / watch / pass against a rubric; state the reason with its cost.', output: 'VERDICT section', guard: 'Must be falsifiable — flags its own valuation sensitivity.' },
        zh: { name: '判斷評分', input: '前述全部輸出', method: '依 rubric 給 track / watch / pass 傾向；說明有成本的理由。', output: 'VERDICT 段', guard: '必須可被反駁——標出自己的估值敏感度。' },
    },
    {
        id: 'S7', key: 'BILINGUAL', section: null,
        en: { name: 'Bilingual pass', input: 'Six drafted sections', method: 'Parallel EN/繁中 generation, numbers held identical across languages.', output: 'Bilingual six-section note', guard: 'A number or proper noun may not drift between languages.' },
        zh: { name: '雙語產出', input: '六段草稿', method: 'EN／繁中平行產出，數字在兩個語言間保持一致。', output: '雙語六段註記', guard: '數字或專有名詞不得因翻譯漂移。' },
    },
    {
        id: 'S8', key: 'STYLE-GUARD', section: null,
        en: { name: 'Style guard', input: 'Bilingual draft', method: 'Scan for AI-voice tells (list-of-three padding, empty adjectives); align to analyst register.', output: 'Publish-ready draft', guard: 'Every remaining claim must trace back to S1 evidence.' },
        zh: { name: '文風守門', input: '雙語草稿', method: '掃描 AI 腔調痕跡（排比堆砌、空泛形容詞）；對齊分析師語氣。', output: '可發佈草稿', guard: '每個留下的主張都必須能回指到 S1 的證據。' },
    },
];

const COPY = {
    en: {
        title: 'The AI strategist: a skill-module system',
        lead: "Deep writing does not scale by hand forever. This is the agent I designed to write an M05-shaped note without losing the depth — eight small, independently checkable skills, not one prompt asking for a whole analyst note at once. It is a concept, not a shipped feature: the draft below is the real, published note for the selected company, replayed section by section against the skill that would have written it.",
        pick: 'Pick a company',
        ...
        simulateThin: 'Simulate thin evidence at the gate',
        draftTitle: 'Assembling the note',
        draftEmpty: 'Run the pipeline to watch the note assemble, section by section.',
        readyBadge: 'Ready for editorial review →',
        soWhat: 'Eight checkable skills beat one giant prompt for the same reason eight quality gates beat one "looks fine" review — you can see exactly which step to fix when the output is wrong.',
    },
    zh: {
        title: 'AI 策略師：技能模組系統',
        lead: '深度寫作沒辦法永遠靠人力擴大規模。這是我設計來寫出 M05 那種深度、卻不流失深度的 agent——八個小型、各自可獨立檢查的技能，而不是一個要求一次生出整篇分析師註記的 prompt。這是概念設計，不是已上線功能：下方的草稿是所選公司真實已發佈的註記，依照原本負責撰寫它的技能逐段重播。',
        pick: '選一家公司',
        run: '執行 pipeline', reset: '重設', running: '執行中…',
        gateLabel: '信心與證據閘門',
        gateHold: '證據不足——保留，退回 S1',
        gatePass: '證據充分——進入雙語與文風守門',
        simulateThin: '模擬閘門處證據不足',
        draftTitle: '組成草稿中',
        draftEmpty: '執行 pipeline，看草稿如何逐段組成。',
        readyBadge: '已可交付編輯審核 →',
        soWhat: '八個可檢查的技能，勝過一個巨大的 prompt，理由跟八個品質閘門勝過一個「看起來還行」的審查一樣——當輸出出錯時，你能精確指出該修哪一步。',
    },
};

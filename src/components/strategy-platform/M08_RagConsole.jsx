...
const QUERIES = [
    {
        id: 'q1', type: 'single',
        en: { q: "What is Cognition AI's biggest risk?", a: 'The clearest risk in the record is the competitive landscape: "Extremely competitive landscape (Microsoft/GitHub, OpenAI, Anthropic, Replit, Cosine, etc.) compressing margins and differentiation." A close second is technical/reliability risk from public demo skepticism.', cites: [{ row: cog.name, field: 'KEY RISKS' }] },
        zh: { q: 'Cognition AI 最大的風險是什麼？', a: '紀錄中最明確的風險是競爭格局：「極度競爭的市場（Microsoft/GitHub、OpenAI、Anthropic、Replit、Cosine 等）壓縮利潤與差異化」。緊追其後的是公開展示引發質疑帶來的技術／可靠度風險。', cites: [{ row: cog.name, field: 'KEY RISKS' }] },
    },
    {
        id: 'q2', type: 'compare',
...
const COPY = {
    en: {
        ...
        lead: "Not a chat box — a retrieval design over the same six-section schema. I chose questions on purpose: one single-company lookup, one cross-company comparison, one cross-document link back to the /insights briefs, and one the database genuinely cannot answer — because a RAG layer that never says 'I don't know' is not treated as governance-grade here.",
        chunkTitle: 'Chunking & retrieval design',
        chunks: [
            { k: 'Chunking', v: 'Each row\'s six sections become typed, citable units — company-fact, moat/risk narrative, sector aggregate, cross-company comparison — not one blob per company.' },
            { k: 'Hybrid retrieval', v: 'Structured filters (sector, date, risk type) narrow the candidate set first; semantic re-ranking over moat/model/risk text picks the final passages.' },
            { k: 'Cross-document link', v: 'The same retrieval index spans /startups rows and /insights briefs, so a question can connect a macro thesis to the specific companies it touches.' },
        ],
        ...
        evalTitle: 'How I would evaluate it',
        evalLead: 'A small gold-set — question in, expected cited row out — so "sounds right" is not the bar. Citation accuracy is graded, not fluency.',
        evalRows: [
            { q: t => t.pin1, expect: cog.name, result: 'pass' },
            { q: t => t.pin2, expect: fig.name, result: 'pass' },
            { q: t => t.pin4, expect: '—', result: 'refuse (correct)' },
        ],
        selfRef: 'The /insights brief dated 2025-12-18 — "Enterprise semantic search is moving from RAG demos to governance-grade systems" — is the thesis this module is built to satisfy: cited, evaluated, and willing to refuse.',
        soWhat: 'The hardest part of RAG is not retrieval. It is the fourth question — the one where the honest answer is "the schema does not have that."',
    },
    zh: {
        title: '架在公司資料庫上的依據式 RAG',
        lead: '不是一個聊天框——而是架在同一套六段式 schema 上的檢索設計。我刻意選了四種問題：一個單一公司查詢、一個跨公司比較、一個連回 /insights 簡報的跨文件連結，以及一個資料庫真的答不出來的問題——因為一個從不說「我不知道」的 RAG 層，在這裡不會被當作治理級。',
        chunkTitle: '切塊與檢索設計',
        chunks: [
            { k: '切塊', v: '每一列的六段變成有型別、可引用的單元——公司事實、護城河／風險敘事、產業聚合、跨公司比較——而不是每家公司一整坨文字。' },
            { k: '混合檢索', v: '結構化過濾（產業、日期、風險型別）先縮小候選集；再對護城河／商模／風險文本做語意重排，選出最終段落。' },
            { k: '跨文件連結', v: '同一個檢索索引橫跨 /startups 列與 /insights 簡報，讓一個問題能把一個宏觀論點連到它實際觸及的公司。' },
        ],
        ...
        evalTitle: '我會怎麼評測它',
        evalLead: '一個小型 gold-set——問題進、期望被引用的列出——讓「聽起來對」不是及格線。評的是引用正確率，不是流暢度。',
        evalRows: [
            { q: t => t.pin1, expect: cog.name, result: '通過' },
            { q: t => t.pin2, expect: fig.name, result: '通過' },
            { q: t => t.pin4, expect: '—', result: '拒答（正確）' },
        ],
        selfRef: '2025-12-18 那篇 /insights 簡報——「Enterprise semantic search is moving from RAG demos to governance-grade systems」——正是這個模組想滿足的論點：有引用、有評測、願意拒答。',
        soWhat: 'RAG 最難的部分不是檢索，而是第四個問題——那個誠實的答案是「schema 裡沒有這個」的問題。',
    },
};

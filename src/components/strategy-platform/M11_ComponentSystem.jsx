...
const COPY = {
    en: {
        title: 'Component & state system',
        lead: 'Five components carry almost the entire interface. None of them are exotic — the craft is in how few states each one needs to feel complete.',
        specimens: [
            { id: 'marker', name: 'Hub marker', states: ['idle', 'selected'], note: 'A ring plus a filled dot — selection reads even at globe scale, without a label.' },
            { id: 'row', name: 'Expandable table row', states: ['collapsed', 'expanded'], note: 'Expansion happens in place; the table underneath never disappears.' },
            { id: 'chip', name: 'Sector filter chip', states: ['inactive', 'active'], note: 'One accent border is the entire "on" state — no fill, no shadow, no motion required.' },
            { id: 'lang', name: 'Language toggle', states: ['EN', '中文'], note: 'A pill, not a dropdown — two languages never need more than two taps.' },
            { id: 'nav', name: 'Floating nav', states: ['idle', 'hover'], note: 'Three icons stay pinned across all three routes — home, insights, database, always in reach.' },
        ],
        soWhat: 'A design system this small only works if every state pulls real weight — there is nowhere to hide an unnecessary one.',
    },
    zh: {
        title: '元件與狀態系統',
        lead: '五個元件撐起幾乎整個介面。沒有一個是特別的——工藝在於每個元件需要多少種狀態才能感覺完整。',
        specimens: [
            { id: 'marker', name: 'Hub marker', states: ['未選取', '已選取'], note: '一個外環加一個實心點——即使在地球儀尺度，選取狀態也一眼可辨，不需要文字標籤。' },
            { id: 'row', name: '可展開表格列', states: ['收合', '展開'], note: '展開發生在原地；底下的表格永遠不會消失。' },
            { id: 'chip', name: '產業篩選 chip', states: ['未啟用', '已啟用'], note: '一條強調色邊框就是完整的「開啟」狀態——不需要填色、陰影或動畫。' },
            { id: 'lang', name: '語言切換', states: ['EN', '中文'], note: '一個藥丸型切換，不是下拉選單——兩種語言永遠不需要超過兩次點擊。' },
            { id: 'nav', name: '浮動導覽', states: ['未觸碰', 'hover'], note: '三個圖示固定在所有三條路徑上——首頁、insights、資料庫，永遠觸手可及。' },
        ],
        soWhat: '這麼小的設計系統之所以行得通，是因為每個狀態都真的有用——沒有地方能藏一個不必要的狀態。',
    },
};

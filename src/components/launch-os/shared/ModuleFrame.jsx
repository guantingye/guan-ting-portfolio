...
.los-body { font-size: 16px; line-height: 1.65; color: var(--text-2); }
.los-data-md { font-family: var(--font-data); font-size: 14px; line-height: 1.5; }
.los-data-sm { font-family: var(--font-data); font-size: 12px; line-height: 1.5; letter-spacing: 0.06em; }
...
.los-role-map { display: flex; gap: 8px; flex-wrap: wrap; }
.los-role-chip {
  font-family: var(--font-data);
  font-size: 11px;
...
.los-signature::before { content: '◇ '; color: var(--amber); }
@media (max-width: 767px) {
  .los-module { padding: 20px 16px; }
  .los-frame-foot { flex-direction: column; }
  .los-signature { text-align: left; }
}
...
// ---- frame (spec 3.4) -----------------------------------------------------
export default function ModuleFrame({ id, eyebrow, title, lead, context, roles = [], signature, children }) {
    return (
        <section className="los los-module" id={id} aria-labelledby={`${id}-title`}>
            <header>
                <div className="los-frame-topline">
                    ...
                    <div className="los-role-map" aria-label="Target roles">
                        {roles.map(role => <span className="los-role-chip" key={role}>{role}</span>)}
                    </div>
                </div>
                <h3 className="los-display-md" id={`${id}-title`}>{title}</h3>
                <p className="los-body los-frame-lead">{lead}</p>
                {context && <p className="los-data-sm los-frame-context">{context}</p>}
            </header>
            ...
            <footer className="los-frame-foot">
                <DataProvenance />
                {signature && <span className="los-signature">{signature}</span>}
            </footer>
        </section>
    );
}

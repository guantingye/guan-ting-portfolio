...
const COPY = {
    en: {
        spotTitle: 'One form never fits a person in crisis',
        before: 'BEFORE · one form for everyone',
        after: 'AFTER · routed by case type',
        redesign: 'redesigned',
        beforeNotes: ['11 fields, identical for every visitor', 'Ordered by admin logic, not user logic', 'People in distress gave up halfway'],
        afterNotes: ['Choose the situation first', 'Grouped fields, visible progress', 'The crisis path asks the least'],
        caption: 'Reconstructed from the intake-form redesign at the center: the “before” asked everyone everything; the redesign starts from the case type and shows only what that path needs — and the crisis path is the shortest of all. Tap the case types to see each path.',
    },
    zh: {
        spotTitle: '「一張表單通用」從不適用於危機中的人',
        before: '改版前 · 人人同一張',
        after: '改版後 · 依個案分流',
        redesign: '重新設計',
        beforeNotes: ['11 個欄位，對每個人都一樣', '順序照行政邏輯，不是使用者邏輯', '狀態不好的人，填到一半就放棄'],
        afterNotes: ['先選情境，欄位才出現', '欄位分組，看得見進度', '危機路徑，問得最少'],
        caption: '重繪心衛中心的諮詢諮商表單改版：改版前，每個人都要答完所有欄位；改版後，先選情境、只問這條路需要的——而危機那條路，是最短的一條。點上方的個案類型，看看每條路徑長什麼樣。',
    },
};

const CASE_TYPES = [
    { key: 'child', label: { en: 'Minor', zh: '兒少' } },
    { key: 'adult', label: { en: 'Adult', zh: '成人' } },
    { key: 'family', label: { en: 'Family', zh: '家庭' } },
    { key: 'crisis', label: { en: 'Crisis', zh: '危機' } },
];
const AFTER_GROUPS = {
    child: [
        { title: { en: 'Basics', zh: '基本資料' }, fields: 2 },
        { title: { en: 'School & care', zh: '就學與照顧' }, fields: 2 },
        { title: { en: 'Main concern', zh: '主要困擾' }, fields: 1 },
    ],
    adult: [
        { title: { en: 'Basics', zh: '基本資料' }, fields: 2 },
        { title: { en: 'Current stressors', zh: '目前壓力來源' }, fields: 2 },
        { title: { en: 'Help you hope for', zh: '期望的協助' }, fields: 1 },
    ],
    family: [
        { title: { en: 'Household', zh: '家庭成員' }, fields: 2 },
        { title: { en: 'Relationship issues', zh: '關係議題' }, fields: 2 },
        { title: { en: 'Help you hope for', zh: '期望的協助' }, fields: 1 },
    ],
    crisis: [
        { title: { en: 'Immediate safety', zh: '立即安全' }, fields: 1, urgent: true },
        { title: { en: 'How to reach you', zh: '怎麼聯絡你' }, fields: 1 },
    ],
};

function MockField({ urgent = false }) {
    return (
        <div className={`fj-mf${urgent ? ' is-urgent' : ''}`} aria-hidden="true">
            <i className="fj-mf-label" />
            <i className="fj-mf-input" />
        </div>
    );
}
...
function FormRedesign() {
    ...
    const [caseType, setCaseType] = useState('crisis');
    const groups = AFTER_GROUPS[caseType];
    return (
        <div className="fj-forms">
            <div className="fj-form fj-form--before">
                <span className="fj-form-head">{c.before}</span>
                <div className="fj-form-sheet">
                    {Array.from({ length: 11 }, (_, i) => <MockField key={i} />)}
                </div>
                <ul className="fj-form-notes">
                    {c.beforeNotes.map((n, i) => <li key={i} data-mark="×">{n}</li>)}
                </ul>
            </div>

            <div className="fj-form-arrow" aria-hidden="true">
                <svg viewBox="0 0 56 90" focusable="false">
                    <DrawnPath d="M8 22 C 34 30, 40 52, 26 72" stroke="var(--fj-accent-ink)" strokeWidth="2" duration={0.9} />
                    <path d="M20 64 L26 74 L34 66" fill="none" stroke="var(--fj-accent-ink)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
                <span>{c.redesign}</span>
            </div>

            <div className="fj-form fj-form--after">
                <span className="fj-form-head">{c.after}</span>
                <div className="fj-form-sheet">
                    <div className="fj-form-cases" role="group" aria-label={c.after}>
                        {CASE_TYPES.map(ct => (
                            <button
                                key={ct.key}
                                className={`fj-form-case${caseType === ct.key ? ' is-on' : ''}`}
                                aria-pressed={caseType === ct.key}
                                onClick={() => setCaseType(ct.key)}
                            >
                                {ct.label[lang]}
                            </button>
                        ))}
                    </div>
                    <div className="fj-form-progress" aria-hidden="true">
                        {groups.map((_, i) => <i key={i} className={i === 0 ? 'is-on' : ''} />)}
                    </div>
                    {groups.map((g, i) => (
                        <div key={i} className={`fj-form-group${g.urgent ? ' is-urgent' : ''}`}>
                            <span className="fj-form-group-title">{g.title[lang]}</span>
                            {Array.from({ length: g.fields }, (_, j) => <MockField key={j} urgent={g.urgent} />)}
                        </div>
                    ))}
                </div>
                <ul className="fj-form-notes">
                    {c.afterNotes.map((n, i) => <li key={i} data-mark="✓">{n}</li>)}
                </ul>
            </div>
        </div>
    );
...
export default function C05_StationThree() {
    const { lang } = useI18n();
    const c = COPY[lang] ?? COPY.en;
    return (
        <StationChapter
            ...
            spotlight={<FormRedesign />}
            spotlightCaption={c.caption}
        />
    );
}

injectStyles('fj-c05-styles', `

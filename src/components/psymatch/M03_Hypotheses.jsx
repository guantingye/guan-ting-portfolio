...
const COPY = {
    en: {
        title: 'Hypotheses & pre-defined success criteria',
        lead: 'Thresholds written down before any data existed — the point of the module.',
        soWhat: 'Success was specified before data existed.',
        stamp: 'PRE-REGISTERED · DEFINED BEFORE DATA',
        cols: ['Hypothesis', 'Metric', 'Threshold', 'Measurement plan'],
        rows: [
            ['H1 — the recommendation is accepted', 'Person selects / books the top-ranked psychologist', '> 20% (chance among the five shown)', 'Log selected rank vs displayed rank; compare against a uniform baseline'],
            ['H2 — completion survives the length', 'Share of intake starts that reach a result', '≥ 70%', 'Funnel: start → each step → submit; record drop-off per step'],
            ['H3 — the fatigue budget holds', 'Median time from step 1 to submit', '≤ 8 minutes', 'Timestamp start and submit; report median and inter-quartile range'],
        ],
        note: 'These are targets, not findings. Module M13 reports the pilot against exactly these three rows.',
    },
    zh: {
        title: '假設與事先定義的成功標準',
        lead: '在任何資料出現之前就寫下門檻——這正是本模組的重點。',
        soWhat: '成功標準在資料存在之前就已明定。',
        stamp: '事前登錄 · 資料出現前即定義',
        cols: ['假設', '指標', '門檻', '量測計畫'],
        rows: [
            ['H1 — 推薦被接受', '使用者選擇／預約排名第一的心理師', '> 20%（五個顯示選項中的隨機機率）', '記錄被選排名對顯示排名；對照均勻基準比較'],
            ['H2 — 長度不擊垮完成率', '開始量表後抵達結果的比例', '≥ 70%', '漏斗：開始 → 各步驟 → 送出；記錄各步驟流失'],
            ['H3 — 疲勞預算守得住', '從第一步到送出的中位時間', '≤ 8 分鐘', '記錄開始與送出時間戳；回報中位數與四分位距'],
        ],
        note: '這些是目標，不是發現。模組 M13 會針對這三列回報試辦結果。',
    },
};

export default function M03_Hypotheses() {
    const { lang } = useI18n();
    const c = COPY[lang] ?? COPY.en;
    return (
        <SectionModule mod={MOD} sectionNo={MOD.no} title={c.title} lead={c.lead} soWhat={c.soWhat}>
            <div className="pm-h3-stamp"><span className="pm-tag pm-tag--amber">{c.stamp}</span></div>
            <div className="pm-table-wrap">
                <table className="pm-table pm-table--reg">
                    <thead>
                    ...
                    <tbody>
                        {c.rows.map((r, i) => (
                            <tr key={i}>
                                {r.map((cell, j) => (
                                    <td key={j} data-label={c.cols[j]} className={j === 0 ? 'pm-table-lead' : undefined}>{cell}</td>
                                ))}
                            </tr>
                        ))}
                    </tbody>
            ...
            <p className="pm-h3-note">{c.note}</p>
        </SectionModule>
    );
}

// Shared table styles (idempotent id → reused by M4, M12, M13).
injectStyles('pm-table', `

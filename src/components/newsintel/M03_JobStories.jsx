...
const COPY = {
    en: {
        ...
        lead: 'Six job stories, three personas, one honest contract. Each story carries a shipping verdict — and the cut ones say why they were cut.',
        actorLabels: { analyst: 'Analyst', lead: 'Team lead', policy: 'Policy researcher' },
        stories: [
            { actor: 'analyst', when: 'I open my laptop at 07:30', want: 'the overnight tech signal already deduped and tagged', outcome: 'I brief the team by 09:00 instead of assembling until noon', status: 'SHIPPED' },
            { actor: 'analyst', when: 'a story quotes a funding number', want: 'the figure quoted verbatim with its source link', outcome: 'I never pass on a hallucinated number', status: 'SHIPPED' },
            { actor: 'analyst', when: 'I read an English scoop', want: 'a faithful zh-TW summary beside the original', outcome: 'the team reads in one language without losing nuance', status: 'SHIPPED' },
            { actor: 'lead', when: 'I scan the week', want: 'items grouped by the taxonomy we actually use', outcome: 'I see where the sector is moving, not just what is loud', status: 'SHIPPED' },
            { actor: 'lead', when: 'something looks urgent', want: 'a confidence signal on the classification', outcome: 'I know when to trust the tag and when to check', status: 'DEFERRED', reason: 'Confidence scoring shipped as a flag, not a calibrated score — needs a bigger eval set first.' },
            { actor: 'policy', when: 'I write a quarterly memo', want: 'sentiment scored per company', outcome: 'I quantify momentum', status: 'CUT', reason: 'Sentiment on tech-news headlines is noisy and easy to over-trust; out of scope for v1.' },
        ],
        contractLabel: 'Scope contract — what v1 will NOT do',
        contract: [
            'No sentiment scoring (headlines are too noisy to trust)',
            'No English-only briefings (bilingual is the point)',
            'No push notifications (the desk is a morning read, not an alert stream)',
            'No user accounts (single internal team, no auth surface)',
            'No mobile app (responsive web is enough for the actual use)',
        ],
        soWhat: "I define what I'm not building before building.",
    },
    zh: {
        title: 'Job stories 與範圍契約',
        lead: '六個 job story、三種角色、一份誠實契約。每個故事都帶著出貨判定——被砍的也說明為什麼被砍。',
        actorLabels: { analyst: '分析師', lead: '組長', policy: '政策研究員' },
        stories: [
            { actor: 'analyst', when: '07:30 打開筆電時', want: '隔夜科技訊號已經去重、標好類', outcome: '我在 09:00 前就能 brief 團隊，而不是組裝到中午', status: 'SHIPPED' },
            { actor: 'analyst', when: '一則新聞引用募資金額', want: '數字逐字引用並附來源連結', outcome: '我絕不轉傳一個幻覺出來的數字', status: 'SHIPPED' },
            { actor: 'analyst', when: '我讀到一則英文獨家', want: '原文旁邊有忠實的繁中摘要', outcome: '團隊用一種語言閱讀，又不失細節', status: 'SHIPPED' },
            { actor: 'lead', when: '我掃過這一週', want: '項目依我們實際使用的分類法分組', outcome: '我看到產業往哪走，而不只是哪裡最吵', status: 'SHIPPED' },
            { actor: 'lead', when: '某件事看起來很急', want: '分類帶一個信心訊號', outcome: '我知道何時信任標籤、何時該查', status: 'DEFERRED', reason: '信心分數目前只做成旗標，不是校準過的分數——需要更大的評測集才行。' },
            { actor: 'policy', when: '我寫季度備忘', want: '每家公司的情緒評分', outcome: '我把動能量化', status: 'CUT', reason: '科技新聞標題的情緒訊號雜且容易被過度信任；不在 v1 範圍。' },
        ],
        ...
        contract: [
            ...
            '不做純英文簡報（雙語才是重點）',
            '不做推播通知（這是早晨閱讀，不是警報流）',
            '不做使用者帳號（單一內部團隊，無需登入層）',
            '不做行動 App（響應式網頁對實際使用已足夠）',
        ],
        soWhat: '我在動手之前，先定義我不做什麼。',
    },
};

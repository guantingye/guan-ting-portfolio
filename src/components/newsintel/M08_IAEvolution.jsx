...
const COPY = {
    en: {
        title: 'Information architecture evolution',
        lead: 'Three versions, from a Notion page to the shipped platform. The v3 tree is not a mockup — it is the real nav on the live product.',
        versions: [
            {
                v: 'v1', era: 'Notion report era',
                tree: [
                    { d: 0, label: 'Daily report (Notion page)' },
                    { d: 1, label: 'All items — one flat list' },
                    { d: 1, label: 'Tags typed by hand' },
                    { d: 1, label: 'EN + ZH mixed in body' },
                ],
            },
            {
                v: 'v2', era: 'Single dashboard',
                tree: [
                    { d: 0, label: 'Dashboard' },
                    { d: 1, label: 'Feed — all items' },
                    { d: 1, label: 'Filter by tag' },
                    { d: 1, label: 'Search' },
                ],
            },
            {
                v: 'v3', era: 'Shipped platform', shipped: true,
                tree: [
                    { d: 0, label: 'Strategy Intelligence Platform' },
                    { d: 1, label: '/ · Home' },
                    { d: 1, label: '/insights · Daily briefings', hot: true },
                    { d: 1, label: '/startups · Deep-tech directory' },
                ],
            },
        ],
        notes: [
            'v1 → v2: a Notion page can’t be filtered or scanned by cadence. Moving to a queryable dashboard made the same content sortable.',
            'v2 → v3: I split the daily read (/insights) from the reference lookup (/startups). People check them on different rhythms, so they became separate routes instead of tabs on one screen.',
        ],
        hotNote: '/insights is where this news pipeline publishes.',
        soWhat: 'The IA changed for stated reasons, not taste.',
    },
    zh: {
        title: '資訊架構演進',
        lead: '三個版本，從一頁 Notion 到已上線的平台。v3 這棵樹不是示意圖，而是線上產品真正在用的導覽。',
        versions: [
            {
                v: 'v1', era: 'Notion 報告時期',
                tree: [
                    { d: 0, label: '每日報告（Notion 頁）' },
                    { d: 1, label: '所有項目——一條扁平清單' },
                    { d: 1, label: '手打標籤' },
                    { d: 1, label: '內文中英混排' },
                ],
            },
            ...
            {
                v: 'v3', era: '已上線平台', shipped: true,
                tree: [
                    { d: 0, label: 'Strategy Intelligence Platform' },
                    { d: 1, label: '/ · Home' },
                    { d: 1, label: '/insights · 每日簡報', hot: true },
                    { d: 1, label: '/startups · 深科技目錄' },
                ],
            },
        ],
        notes: [
            'v1 → v2：Notion 頁無法篩選、也難依節奏掃描。改成可查詢的儀表板後，同樣內容變得可排序。',
            'v2 → v3：把每天要讀的簡報（/insights）和偶爾查閱的目錄（/startups）拆開。大家看這兩者的節奏不同，所以做成各自的路徑，而不是同一頁的分頁。',
        ],
        hotNote: '/insights 正是這條新聞管線的發布之處。',
        soWhat: '資訊架構是為了明說的理由而改，不是憑喜好。',
    },
};
